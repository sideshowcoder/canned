  ...
  if (fileObject) {
    ...
    fs.readFile(filePath, { encoding: 'utf8' }, function (err, data) {
      ...
      if (err) {
        ...
        that._extractRequestContent(httpObj);
...
Canned.prototype._extractRequestContent = function (httpObj) {
  var that = this
  var body = ''

  // assemble response body if GET/POST/PUT
  switch(httpObj.req.method) {
  case 'PUT':
  case 'POST':
    httpObj.req.on('data', function (data) {
      body += data
    })
    httpObj.req.on('end', function () {
      var responderBody = querystring.parse(body);
      if (isContentTypeJson(httpObj.req)) {
        try {
      ...
      httpObj.content = responderBody;
    })
    break
  case 'GET':
    var query = url.parse(httpObj.req.url).query
